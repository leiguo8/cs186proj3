...
public class Database implements AutoCloseable {
    ...
    void lockTableMetadata(String tableName, LockType lockType) {
        ...
        RecordId row = tableInfoLookup.get(tableName);
        LockUtil.ensureSufficientLockHeld(getTableInfoContext().childContext(row.getPageNum()), lockType);
    ...
    void lockIndexMetadata(String indexName, LockType lockType) {
        ...
        RecordId row = indexInfoLookup.get(indexName);
        LockUtil.ensureSufficientLockHeld(getIndexInfoContext().childContext(row.getPageNum()), lockType);
    ...
    private class TransactionContextImpl extends AbstractTransactionContext {
        ...
        public void close() {
            ...
            List<Lock> holdLocks = getLockManager().getLocks(this);
            for(int i = holdLocks.size() - 1; i >= 0; i--){
                Lock lock = holdLocks.get(i);
                LockContext currContext = LockContext.fromResourceName(getLockManager(),lock.name);
